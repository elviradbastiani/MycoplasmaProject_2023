...
####################################################################################################
#Script S4: Separating the 14 days of movement ecology data for griffon vultures prior to sampling.
####################################################################################################
...
mycoplasma_data <- read.csv2("myco_parasite_results_2021_edb_06_sep_2023.csv", sep = ",")
dim(mycoplasma_data)
...
# Step 14: Define the distThreshold
distance=25 #10, 25, 50, 75, and 100 meters

# Step 15: Define the windows of movement of vultures before sampling date
window=3 #7,14,21,28 days
...
for (j in 1:length(window)) {#loop - make for all windows
  for (d in 1:length(distance)) {#loop - make for all windows
    for (ii in 1:length(list_myDates)) {#loop - make for all dates 
